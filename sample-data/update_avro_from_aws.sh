...
for path in $(aws s3 --profile brazedev ls ${bucket}${prefix});
do
  if [[ $path != "" ]] && [[ $path != "PRE" ]]; then
    if [[ $path = "event_type=users"* ]]; then
      event_type=$(echo $path | sed 's/event_type=*//g;s/\///g')
      payloadpath=$(aws s3 --profile brazedev ls "${bucket}${prefix}${path}${date}/" --recursive | sort | tail -n 1 | awk '{print $4}')
      if [[ $payloadpath == "" ]]; then
        ...
        payloadpath=$(aws s3 --profile brazedev ls "${bucket}${prefix}${path}${prev_date}/" --recursive | sort | tail -n 1 | awk '{print $4}')
        if [[ $payloadpath == "" ]]; then
          echo "ERROR: Couldn't find data in ${prev_date} either. Exiting."
          exit 1
        fi
      fi
      aws s3 --profile brazedev cp s3://${bucket}${payloadpath} Avro/${event_type}.avro
    fi
  fi
done
